<?php
// Vecteezy.com Proxy Script with Advanced Cloudflare Bypass
// Handles all requests and forwards them to vecteezy.com

error_reporting(0); // Suppress errors for cleaner output

// Target URL
$targetDomain = 'www.vecteezy.com';
$targetScheme = 'https';
...
function refreshCloudflareTokens()
{
    ...
    $ch = curl_init('https://www.vecteezy.com/');
...
if (file_exists($cookieFile)) {
    ...
    foreach ($cookieLines as $line) {
        ...
        if (count($parts) >= 7) {
            ...
            // Only include cookies for vecteezy.com domain and skip expired cookies
            if (strpos($domain, 'vecteezy.com') !== false && ($expiration == 0 || $expiration > time())) {
...
// Add required headers for vecteezy.com and Cloudflare bypass
...
if (
    ...
    $responseBody = str_replace(
        ['"www.vecteezy.com"', "'www.vecteezy.com'"],
